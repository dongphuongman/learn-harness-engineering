...
**有完整儀表盤**，計劃者輸出衝刺合約，列明要改哪些元件、每個元件的驗證標準、排除項（不處理列印樣式）。生成者按合約實現。執行時期可觀測性記錄每個元件的樣式加載和應用過程。評估者用評分標準逐維度評估，附具體證據引用，「按鈕顏色對比度不足（WCAG AA 標準 4.5:1，實測 2.1:1）」。一次迭代出高品質結果，總耗時約 15 分鐘。
