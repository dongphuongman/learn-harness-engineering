...
倉庫路徑：`projects/project-02/`

| 目錄 | 內容 | 比較什麼 |
|------|------|------|
| `starter/` | 基於 Project 01 solution 的代碼，但導入/詳情/持久化仍未完整；文檔更薄，且缺少 `session-handoff.md`。 | 新開會話時，agent 需要做多少重新探索才能接上進度。 |
| `solution/` | 同一產品切片已完成，並在 `projects/project-02/solution/` 下補齊核心文檔（以及 `feature_list.json`、`session-handoff.md`）等可接手產物。 | 只依靠倉庫狀態，能否快速續寫而不靠口頭上下文。 |
