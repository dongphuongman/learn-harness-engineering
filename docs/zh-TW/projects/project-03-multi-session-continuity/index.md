...
倉庫路徑：`projects/project-03/`

| 目錄 | 內容 | 比較什麼 |
|------|------|------|
| `starter/` | Project 02 代碼基礎上，索引與 grounded QA 尚未完善；有初版 `feature_list.json`，但缺少最終的重啟/交接與乾淨狀態產物。 | 多次重啟/多功能任務下，agent 是否漂移、是否丟狀態。 |
| `solution/` | 完整的 chunking、metadata、indexing status、citation-based QA，並補齊 `init.sh`、`session-handoff.md`、`claude-progress.md`、`clean-state-checklist.md`。 | 是否能做到“一次只做一個功能”，且每個功能都有可執行證據才標記通過。 |
