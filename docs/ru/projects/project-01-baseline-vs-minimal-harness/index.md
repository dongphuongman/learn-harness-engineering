...
Путь в репозитории: `projects/project-01/`

| Каталог | Что внутри | Как использовать / что сравнивать |
|------|------|------|
| `starter/` | Запуск со слабым harness. Есть только `task-prompt.md` как описание задачи, без `AGENTS.md` и `feature_list.json`. | Дайте агенту prompt и измерьте, что он выполнит без дополнительной структуры. |
| `solution/` | Тот же срез продукта с явными harness-артефактами: `AGENTS.md`, `CLAUDE.md`, `init.sh`, `feature_list.json`, `claude-progress.md`. | Сравните, как правила и проверка делают ту же задачу конкретной и проверяемой. |
