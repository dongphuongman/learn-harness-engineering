...
Путь: `projects/project-02/`

| Каталог | Что внутри | Что сравнивать |
|------|------|------|
| `starter/` | Код Project 01, где импорт документов, детальная страница и persistence ещё не завершены. Документация есть, но она тоньше, и нет `session-handoff.md`. | Сколько контекста второй сессии агента приходится открывать заново. |
| `solution/` | Тот же срез продукта завершён; документы для передачи состояния находятся в `projects/project-02/solution/`, также есть `feature_list.json` и `session-handoff.md`. | Может ли новая сессия продолжить работу только по состоянию репозитория. |
