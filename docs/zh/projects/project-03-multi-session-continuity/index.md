...
仓库路径：`projects/project-03/`

| 目录 | 里面有什么 | 比较什么 |
|------|------|------|
| `starter/` | Project 02 代码，索引和带引用问答未完成。有 starter 版 `feature_list.json`，但缺少最终重启/交接产物。 | agent 是否跨多个功能漂移，重启后是否丢状态。 |
| `solution/` | 完成文档分块、元数据、索引状态、带引用问答，并加入 `init.sh`、`session-handoff.md`、`claude-progress.md`、`clean-state-checklist.md`。 | 每个功能是否有验证证据后才标为 passing。 |
